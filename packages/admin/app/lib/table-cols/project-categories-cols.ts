...
type RecordColType = FetchOrgProjectCategoryResponseType['data'][number];
type Props = {
  editRecord?: (row: RecordColType) => void;
  deleteRecord?: (row: RecordColType['id']) => void;
};

export function getOrgProjectCategoriesTableCols(props: Props) {
  const orgProjectCategoriesTableCols: ColumnDef<RecordColType>[] = [
    {
  ...
  return orgProjectCategoriesTableCols;
}

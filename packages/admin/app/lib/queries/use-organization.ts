...
export const useOrganization = function () {
  ...
  const [currentOrg, setCurrentOrg] =
    useState<SelectOrganizationInterface | null>(null);
  ...
  async function fetchOrganizations() {
    ...
    return decodeArrayBuffer<ResponseObject<SelectOrganizationInterface[]>>(
      resp.data,
    ).data;
  }
  ...
  const setCurrentOrganization = (
    ...
    authStore.setUserCurrentOrganization(organization, {
      onSuccess: () => {
        const org = query.data?.find((org) => org.uuid === organization);
  ...
  const createOrgMutation = useMutation({
    ...
    onError: (error) => {
      throw new Error(
        extractAxiosError(error) ?? 'Error creating organization',
      );
    },
    onSuccess: () => {

...
export function useLayout() {
  function setPrimary(value: TsFixMeType) {
    appState.value.primary = value
  }

  function setSurface(value: TsFixMeType) {
    appState.value.surface = value
  }

  function setDarkMode(value: TsFixMeType) {
  ...
  function updatePrimaryColors(palette: TsFixMeType) {
  ...
  function updateSurfaceColors(palette: TsFixMeType) {
    const root = document.documentElement
    Object.entries(palette).forEach(([key, value]) => {
      root.style.setProperty(`--p-surface-${key}`, value as string)
    })
  }

  watch(
    [() => appState.value.darkMode, () => appState.value.primary, () => appState.value.surface],
    () => {
      localStorage.setItem('primevue-appconfig', JSON.stringify(appState.value))
    },
    { deep: true },
  )
  ...
  if (storedConfig) {
    if (appState.value.primary) {
      const color = primaryColors.value.find((c) => c.name === appState.value.primary)

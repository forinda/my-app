...
<template>
  <div class="text-input-container w-full">
    ...
    <div
      ...
      <!-- Input Field -->
      <Field v-if="as !== 'select'" :type="type ?? 'text'" :as="as ?? 'input'" :id="id ?? name" :name="name"
        :autocomplete="autoComplete" :pattern="pattern" :placeholder="placeholder" :aria-label="labelText || name"
        class="input-field w-full px-3 py-2 focus:outline-none placeholder-gray-400" />
      <Field v-else :type="as" :id="id ?? name" :name="name" :autocomplete="autoComplete" :pattern="pattern"
        :placeholder="placeholder" :aria-label="labelText || name"
        class="input-field w-full px-3 py-2 focus:outline-none placeholder-gray-400">
